...
using UnityEngine;
	...
	public class PlayerControl : MonoBehaviour
	{
		...
		void Start()
		{
			...
			canPerfectDodge = GetComponentInChildren<perfectDodge>().PerfectDodge();
		}
		...
		{	
            ...
            moveInput = inputManager.MoveInput.x;
			if (isGrounded)
			{
				m_extraJumps = extraJumpCount;
			}
			...
			// Jumping
			if(inputManager.JumpTriggered && m_extraJumps > 0 && !isGrounded && !m_wallGrabbing)	// extra jumping
			{
			...
			else if(inputManager.JumpTriggered && (isGrounded || m_groundedRemember > 0f))	// normal single jumping
			{
				m_rb.linearVelocity = new Vector2(m_rb.linearVelocity.x, jumpForce);
				// jumpEffect
				PoolManager.instance.ReuseObject(jumpEffect, groundCheck.position, Quaternion.identity);
			}
			else if(inputManager.JumpTriggered && m_wallGrabbing && moveInput!=m_onWallSide )		// wall jumping off the wall
			{
				m_wallGrabbing = false;
				m_wallJumping = true;
			...
			else if(inputManager.JumpTriggered && m_wallGrabbing && moveInput != 0 && (moveInput == m_onWallSide))      // wall climbing jump
			{
				m_wallGrabbing = false;
				m_wallJumping = true;
